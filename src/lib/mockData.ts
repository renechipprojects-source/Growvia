...
export const REVENUE_MONTHLY: { month: string; revenue: number; admissions: number }[] = [];
export const BRANCH_STATS: { branch: string; students: number; teachers: number; revenue: number }[] = [];
export const CLASS_DIST: { name: string; value: number; color: string }[] = [];

export const ANNOUNCEMENTS: { id: number; title: string; date: string; body: string }[] = [];
export const EVENTS: { id: number; title: string; date: string; type: string; color: string }[] = [];
export const DIARY: { id: number; date: string; note: string; mood: string }[] = [];
export const ATTENDANCE_TREND: { day: string; present: number }[] = [];
export const BIRTHDAYS: { name: string; className: string; avatar: string; date: string }[] = [];
...
export interface Message {
  id: string;
  fromId: string; // teacher id / "office" / "principal"
  fromName: string;
  studentId: string; // which student's parents this is about
  toParentId: string; // household id
  subject: string;
  body: string;
  time: string;
  priority: "Normal" | "High";
  read: boolean;
  direction: "outgoing" | "incoming"; // from teacher perspective
}

const msgRand = rng(97);
