    ...
    <div className="flex flex-col h-full min-h-0 space-y-4">
            ...
            {list.map((m) => (
              <li key={m.id}>
                <button
                  ...
                  className={`w-full text-left px-3 py-3 rounded-2xl transition ${selected?.id === m.id ? "bg-indigo-50" : "hover:bg-white/70"}`}
                >
                  <div className="flex items-start gap-3">
                    <span className={`mt-2 h-2 w-2 rounded-full shrink-0 ${!m.read ? "bg-indigo-500" : "bg-slate-300"}`} />
      ...
      <Dialog open={composeOpen} onOpenChange={setComposeOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>New Admin Broadcast / Direct Message</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSendMessage} className="bg-indigo-600 text-white">
    ...
    <div className="flex flex-col h-full min-h-0 p-5 space-y-4">
      <div className="border-b pb-3">
        ...
        <div className="text-xs text-muted-foreground mt-1 flex items-center justify-between">
          ...
          <span className={`font-semibold ${m.read ? "text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-full" : "text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full"}`}>
            {m.read ? "✓ Read" : "○ Unread"}
          </span>
        </div>
      </div>
