...
function OfficeStudents() {
  ...
  const cols: ColumnDef<Student>[] = [
    { header: "Admission No.", accessorKey: "admissionNo", cell: (c) => <span className="font-mono text-xs font-semibold text-slate-700">{toCanonicalAdmissionNo(c.row.original.admissionNo, c.row.original.id)}</span> },
    {
    ...
    <div className="space-y-4 w-full max-w-none">
          ...
          action={
            <div className="flex items-center gap-2">
              <Button
                ...
                variant="outline"
                className="bg-white hover:bg-slate-50 text-indigo-700 border-slate-200 rounded-xl text-xs font-semibold px-3 py-2 shadow-xs"
              >
                <UserCheck className="mr-1.5 h-4 w-4 text-indigo-600" /> Class & Section Allocation
              </Button>
              <Button
      ...
      <Tabs defaultValue="students" className="w-full">
        ...
        <TabsContent value="students" className="mt-3">
          <DataTable data={data} columns={cols} searchKey="name" />
        </TabsContent>
