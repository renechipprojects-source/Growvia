...
async function testAnonQuery() {
  console.log("Testing gv_users query with ANON client (unauthenticated browser simulation)...");
  const { data, error } = await supabaseAnon.from("gv_users").select("*").ilike("login_id", "ADMIN001");
  console.log("Result:", { count: data?.length, error: error?.message, row: data ? data[0] : null });
}
